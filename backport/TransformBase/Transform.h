...
#ifndef BACKPORT_MODERNIZE_TRANSFORM_H
#define BACKPORT_MODERNIZE_TRANSFORM_H
...
struct TransformOptions {
  ...
  /// \brief Specified target compiler version.
  CompilerVersions TargetVersions;

  /// \brief Contains a pointer to the BackportManager.
  backport::BackportManager *BackportMgr;
};
...
public:
  ...
  /// \brief Returns the targeted compiler version
  CompilerVersions getTargetCompilerVersions() const;
  ...
  void addTracedReplacement(const clang::SourceManager& SM, const clang::CharSourceRange& range, const std::string& replacement, int originalLineNumber = -1);

  void addTracedReplacement(const clang::SourceManager& SM, const clang::SourceRange& range, const std::string& replacement);
  ...
  static std::map< std::string, std::set< backport::helper::Path > > TransformationSourceMap;
  const std::string& getCurrentSource() { return CurrentSource; }

  std::set<backport::ReplacementData> replacements;
  std::map<std::string, unsigned int> modifiedFilesCurrentLength;
...
public:
  ...
  bool supportsCompilers(CompilerVersions Target, CompilerVersions Required, bool isNullTrue = true) const;
...
#endif // BACKPORT_MODERNIZE_TRANSFORM_H
